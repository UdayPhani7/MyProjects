<?php
    ...
    if(isset($_GET['id'])){
        $user = mysqli_query($conn, "SELECT * FROM users where uid = '" . $_GET['id'] . "';");
                        ...
                        <form method="post">
                            <div class="row mt-3">
                                <div class="col-md-12">
                                    ...
                                    <input type="text" name="name" class="form-control" placeholder="<?php echo $row['name']; ?>" disabled>
                                </div>
                                <div class="col-md-12">
                                    <label class="labels">Mobile Number</label>
                                    <input type="text" name="phone" class="form-control" placeholder="<?php echo $row['phone']; ?>" disabled>
                                </div>
                                <div class="col-md-12">
                                    <label class="labels">Email ID</label>
                                    <input type="text" name="email" class="form-control" placeholder="<?php echo $row['email']; ?>" disabled>
                                </div>
                                <div class="col-md-12">
                                    <label class="labels">Bio</label>
                                    <input type="text" name="bio" class="form-control" placeholder="<?php echo $row['bio']; ?>" disabled>
                                </div>
                            </div>
                        </form>
                    </div>
...
<?php
                        $query2 = "SELECT * FROM recipe where rid in (SELECT rid FROM connect where uid = " . $row['uid'] . ")";
...
?>
                                ...
                                <u><b><?php echo "<br>".$images['rname'];?></b></u></li></a>
<?php
                            } 
                        }
?>
